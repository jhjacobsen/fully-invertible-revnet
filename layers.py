...
def NN(in_channels, hidden_channels=128, channels_out=None):
    ...
    return nn.Sequential(
        ...
        #spectral_norm_conv(nn.Conv2d(in_channels, hidden_channels, 3, stride=1, padding=1),2.,(in_channels,3,3),n_power_iterations=1),
        nn.ReLU(inplace=True),
        Conv2dActNorm(hidden_channels, hidden_channels, 1, stride=1, padding=0),
        #spectral_norm_fc(nn.Conv2d(hidden_channels, hidden_channels, 1, stride=1, padding=0),2,n_power_iterations=1),
        nn.ReLU(inplace=True),
        nn.Dropout(0.1),
        Conv2dZeroInit(hidden_channels, channels_out, 3, stride=1, padding=1))
        #spectral_norm_conv(Conv2dZeroInit(hidden_channels, channels_out, 3, stride=1, padding=1),2.,(hidden_channels,3,3),n_power_iterations=1))
