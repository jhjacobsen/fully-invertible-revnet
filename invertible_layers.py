...
class Layer(nn.Module):
    ...
    def forward_(self, x, objective, zn):
        raise NotImplementedError

    def reverse_(self, y, objective, zn):
        raise NotImplementedError
...
class LayerList(Layer):
    ...
    def forward_(self, x, objective, zn):
        for layer in self.layers:
            x, objective, zn = layer.forward_(x, objective, zn)
        return x, objective, zn

    def reverse_(self, x, objective, zn):
        for layer in reversed(self.layers): 
            x, objective, zn = layer.reverse_(x, objective, zn)
        return x, objective, zn
...
class Shuffle(Layer):
    ...
    def forward_(self, x, objective, zn):
        return x[:, self.indices], objective, zn

    def reverse_(self, x, objective, zn):
        return x[:, self.rev_indices], objective, zn
...
class Squeeze(Layer):
    ...
    def forward_(self, x, objective, zn):
        ...
        return self.squeeze_bchw(x), objective, zn
        
    def reverse_(self, x, objective, zn):
        if len(x.size()) != 4: 
            raise NotImplementedError

        return self.unsqueeze_bchw(x), objective, zn
...
class Split(Layer):
    ...
    def forward_(self, x, objective, zn):
        ...
        return z1, objective, zn

    def reverse_(self, x, objective, zn, use_stored_sample=False, use_zn=True):
        #pz = self.split2d_prior(x)
        #z2 = self.sample if use_stored_sample else pz.sample() 
        z2 = zn.pop() #if use_zn else pz.sample() 
        z = torch.cat([x, z2], dim=1)
        #objective -= pz.logp(z2) 
        return z, objective, zn
...
class AdditiveCoupling(Layer):
    ...
    def forward_(self, x, objective, zn):
        z1, z2 = torch.chunk(x, 2, dim=1)
        z2 += self.NN(z1)
        return torch.cat([z1, z2], dim=1), objective, zn
...
class ActNorm(Layer):
    ...
    def forward_(self, input, objective, zn):
        ...
        output = (input + b) * (torch.exp(logs)+0.001)
        dlogdet = torch.sum(logs) * input.size(-1) # c x h  

        return output.view(input_shape), objective + dlogdet, zn

    def reverse_(self, input, objective, zn):
        assert self.initialized
        input_shape = input.size()
        input = input.view(input_shape[0], input_shape[1], -1)
        logs = self.logs * self.logscale_factor
        b = self.b
        output = input / (torch.exp(logs)+0.001) - b
        dlogdet = torch.sum(logs) * input.size(-1) # c x h  
...
class Glow_(LayerList, nn.Module):
    ...
    def sample(self, x, zn):
        with torch.no_grad():
            samples = self.reverse_(x, 0., zn)[0]
            return samples
